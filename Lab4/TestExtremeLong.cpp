...
try
{
    ...
    cout << "124123456789012345687932432048203XL*789389838430850348058409385098409845XL=" <<
          124123456789012345687932432048203XL*789389838430850348058409385098409845XL << endl << endl;
    ...
    { ExtremeLong product{1};
      ...
      for (long idx{1};idx<=n;idx++)  product *= idx;
      cout << endl << n << "! = " << product << endl;
      cout << "with " << product.digits() << " digit(s)" <<endl;
    }

    {  cout << endl << "Enter two integers n1 & n2 to evaluate n1!+n2! :" << endl;
        int n1,n2;    cin >> n1 >> n2;
        ExtremeLong product1{1};
        for (long idx{1};idx<=n1;idx++)  product1 *= idx;
        ExtremeLong product2{1};
        for (long idx{1};idx<=n2;idx++)  product2 *= idx;
